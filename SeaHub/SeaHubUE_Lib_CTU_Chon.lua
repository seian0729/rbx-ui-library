...
local s,Translated = pcall(function() 
    return game.HttpService:JSONDecode(readfile("Sea Hub/translateCache.json"))
end)
function SaveFile() 
    pcall(function() 
        writefile("Sea Hub/translateCache.json",game.HttpService:JSONEncode(TranslateCache))
    end)
end
if s and Translated then TranslateCache = Translated end
if GUI then 
    GUI.DescendantAdded:Connect(function(des) 
        if des then 
            if pcall(function() return des.Text end) then 
                delay(0,function() 
                    if des.Text=="" or string.match(des.Text,"Sea Hub") then return end
                    local old = des.Text
                    local c = 0
                    while des.Text==old and c<3 do 
                        SetRandomLang()
                        local t = getgenv().TranslateCache[des.Text] or Translate(des.Text)
...
spawn(function() 
    local t = tick()
    while wait(2) do SaveFile()
    if tick()-t > 30 then return end
    end
end)
...
Main = t.CreateMain({Title = 'Super Idol', Desc = '105 Temperature'})
...
local tvk = false
Section11.CreateButton({Title = 'cac'}, function()
    NaziNuclearToggle:SetStage(not tvk)
    tvk = true
end)

-- NaziNuclearToggle.SetStage(<bool>)
Section11.CreateLabel({Title = 'Fuck LGBT, hitler is love hitler is life \n- i cummed in ur mum pussy \n- sech qua em oi \n- nung nung nung \n- i cummed in ur mum pussy \n- sech qua em oi \n- nung nung nung \n- i cummed in ur mum pussy \n- sech qua em oi \n- nung nung nung'})
...
SelectWhatYouWantTodoDropdown = Section12.CreateDropdown({Title = 'Select what u want todo', List = TodoList, Search = false, Selected = true, Default = nil}, function(i, v)
    if v then
        t.CreateNoti({
            Title = 'Dropdown 1',
