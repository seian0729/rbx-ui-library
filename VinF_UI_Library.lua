...
if getgenv().VinF_Loaded1 then
    game.Players.LocalPlayer:Kick('The Script Already Executed')
else
    getgenv().VinF_Loaded = true
end
...
if syn.protect_gui then 
	syn.protect_gui(Library.NotiGui)
...
function Library.CreateNoti(Setting)
	...
	TextLabel.Text = "<font color=\"rgb("..tostring(getcolor(UI_Theme['Main Color'])[1])..','..tostring(getcolor(UI_Theme['Main Color'])[2])..','..tostring(getcolor(UI_Theme['Main Color'])[3])..")\">VIN HUB</font> - "..TitleName
...
function Library.CreateWindow(WindowName)
	...
	Library.Gui.Name = 'VinF[Premium Edition]'
	...
	TitleName.Text = "<font color=\"rgb("..tostring(getcolor(UI_Theme['Main Color'])[1])..','..tostring(getcolor(UI_Theme['Main Color'])[2])..','..tostring(getcolor(UI_Theme['Main Color'])[3])..")\">VIN HUB</font> - "..WindowName
	...
	VinfText.Text = "<b>Vin Hub Info</b>"
				...
				local Sec_zin = 99
					function Folder_function.AddSection(Section_name)
						...
						function Section_function.CreateDropdown(Setting,Callback)
							...
							DropdownFrame.Name = TitleText.."Ngu"
							DropdownFrame.Parent = SectionName
							DropdownFrame.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
							DropdownFrame.Position = UDim2.new(0, 0, 0.725806475, 0)
							DropdownFrame.Size = UDim2.new(1, 0, 0, 50)
							DropdownFrame.ZIndex = 2
							DropdownFrame.LayoutOrder = OderSec;
							...
							DropdownVal.Position = UDim2.new(0.999999881, -10, 0.5, 0)
							DropdownVal.Size = UDim2.new(0, 200, 0, 24)
							...
							DropdownScroll.AnchorPoint = Vector2.new(0,0)
							...
							DropdownDesc.Position = UDim2.new(0, 10, 0.600000024, -5)
							DropdownDesc.Size = UDim2.new(1, -10, 0.5, 0)
							DropdownDesc.Font = Enum.Font.GothamBold
							DropdownDesc.TextColor3 = Color3.fromRGB(255, 255, 255)
							DropdownDesc.TextSize = 12.000
							DropdownDesc.TextXAlignment = Enum.TextXAlignment.Left
							DropdownDesc.Text = DescText
							...
							DropdownTitle.Size = UDim2.new(1, -10, 0.5, 10)
							...
							if Search_Setting then 
								...
								TextLabel.Focused:Connect(function()
									...
									for i,v in next, SectionName:GetChildren() do
										if v:IsA('Frame') then 
											table.insert(table_abc,v)
										end
									end

									maxcount = table_abc[1].LayoutOrder
									local obj 
									for i,v in next, table_abc do
										if maxcount < table_abc[i].LayoutOrder then
											maxcount = table_abc[i].LayoutOrder
											obj = table_abc[i]
										end 
									end

									if DropdownFrame.LayoutOrder >= maxcount then 
										pos = UDim2.new(0,0,0,-5)
										point = Vector2.new(0,1)
									elseif DropdownFrame.LayoutOrder <= 2 and DropdownFrame.Parent.LayoutOrder == 0 then 
										pos = UDim2.new(0,0,1,5)
										point = Vector2.new(0,0)
										if not isbusy then 
										oldz = SectionName.ZIndex
										--SectionName.ZIndex = SectionName.ZIndex + 1
										end
									else			
										if (obj.AbsolutePosition.Y - DropdownFrame.AbsolutePosition.Y) > 200  then
											pos = UDim2.new(0,0,1,5)
											point = Vector2.new(0,0)
										else
											pos = UDim2.new(0,0,0,-5)
									...
									--DropdownContainer.Position = pos
									--DropdownContainer.AnchorPoint = point
...
Main = Library.CreateWindow('Anime Fighters Simulator')
t1 = Main.CreatePage('Main')
e2 = t1.AddFolder({Folder_Name = 'Main',Folder_Desc = 'Main Function'})
bucu = e2.AddSection('Main')
but2 = bucu.CreateButton({ButtonName = 'Dupe Item', ButtonDesc = 'Dupe Item So luong lon' , ButtonValue = 'Click here de dupe'},function()
	
end)

tog = bucu.CreateToggle({ToggleName = 'Van Ddos', ToggleDesc = 'Van ddos nha trang', DefaultValue = false},function(v)
	print(v)
end)
local eee = {'vanmeo','vandog','kiet handicap'}
local methodskill = {
    ["Z"] = false,
    ["X"] = false,
    ["C"] = false,
...
local eeee = {'vanmeo','vandog','kiet handicap','ngu vai lom','bu cu tao di'}
dr = bucu.CreateDropdown({Title = '123',Desc='347',List = methodskill,Search = true, Selected = true ,DefaultValue = methodskill2},function(i,v)
	methodskill[i] = v
	Library.CreateNoti({Title = 'Van ddos Noti',Desc = i..'='..tostring(v)})
end)

sl = bucu.CreateSlider({Title = '33333',Desc='3333',Min = 0,Max = 100,DefaultValue = 0},function(i,v)
	methodskill[i] = v
	Library.CreateNoti({Title = 'Van ddos Noti',Desc = i..'='..tostring(v)})
end)

but2 = bucu.CreateButton({ButtonName = 'Van ddos', ButtonDesc = 'Ngu vai lon' , ButtonValue = 'Click here de bu cu'},function()
	dr:GetNewList(eeee)
end)

bucu = e2.AddSection('bu cu 2')
tog = bucu.CreateToggle({ToggleName = 'Van Ddos', ToggleDesc = 'Van ddos nha trang', DefaultValue = false},function(v)
	print(v)
end)
local eee = {'vanmeo','vandog','kiet handicap'}
local eeee = {'vanmeo','vandog','kiet handicap','ngu vai lom','bu cu tao di'}
dr = bucu.CreateDropdown({Title = '123',Desc='347',List = eee,Search = true, Selected = false ,DefaultValue = 'kiet handicap'},function(v)
	Library.CreateNoti({Title = 'Van ddos Noti',Desc = v})
end)
...
box2 = bucu.CreateTextBox({Title = 'Van ddos', Desc = 'Ngu vai lon' , DefaultValue = 'kiet handicap'},function(v)
	Library.CreateNoti({Title = 'Van ddos Noti',Desc = v})
end)



Main.CreatePage('Test 1')
Main.CreatePage('Test 2')

--[[
The Final Project before i quited exploit
VinF UI Libary - Designed and Writed by Chim#0889
Finish at 5:35Am - 24/10/21 (JST)
+ Edited 6:25Am - 26/10/21 (JST)
+ Edited 11:25Pm - 28/10/21 (JST)
]]